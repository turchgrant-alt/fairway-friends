...
export default function SectionHeader({ title, action, onAction, description, className }: SectionHeaderProps) {
  return (
    <div className={`flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between ${className ?? ""}`}>
      <div>
        <h2 className="font-display text-lg text-[hsl(var(--golfer-deep))]">{title}</h2>
        {description ? (
          <p className="mt-1 max-w-2xl text-sm leading-6 text-[hsl(var(--golfer-deep-soft))]/[0.76]">{description}</p>
        ) : null}
      </div>
      {action && (
        <button
